...
> **银趴火山帮** 出品 · 从 [VPS 开荒脚本](https://github.com/chnnic/SSH-Hardening) 同步至 V3.12.6 独立提取
...
首次调优会将运行参数保存为权限 `600` 的持久基线；每次应用前保存运行快照。所有待应用参数写入失败、回读不一致或持久化失败时自动恢复本次修改前参数。事务锁防止同时写入，INT/TERM/HUP 中断触发回滚；回滚失败时保留快照。强制终止/断电不保证自动恢复。旧配置若缺少真实基线，不会把当前调优值当成原值。
...
- 偏好与 sysctl 原子保存于 `/etc/sysctl.d/99-vps-bbr.conf`，切换预设后保持。恢复原值会退出本工具管理；没有首次基线时拒绝猜测默认值。
