...
> **银趴火山帮** 出品 · 从 [VPS 开荒脚本](https://github.com/chnnic/SSH-Hardening) 同步至 V3.12.7 独立提取
...
首次调优会将运行参数保存为权限 `600` 的持久基线；每次应用前保存运行快照。所有待应用参数写入失败、回读不一致或持久化失败时自动恢复本次修改前参数。转发事务额外保存 default/各接口的 forwarding、受影响的 IPv4 redirects、IPv6 force_forwarding 与 RA，恢复后整体复核。旧配置若缺少真实基线，不会把当前调优值当成原值；已消失的旧参数或接口恢复项会提示后跳过，权限失败仍会取消事务。

使用 `flock` 内核锁防止同时写入（缺少时安装 `util-linux`）；INT/TERM/HUP 中断触发回滚，回滚失败时保留快照。强制终止/断电不保证参数自动恢复，但锁会随进程退出/重启自动释放。锁文件保留是正常状态，不能删除正在使用的锁文件。从 V3.12.6 升级若遗留的是 `.lock` **目录**，请先确认旧进程已结束，再手动 `rmdir /etc/sysctl.d/99-vps-bbr.conf.lock`；脚本不会擅自清除所有者不明的旧目录锁。

运行应用和持久化文件都先设置 `accept_ra=2` 再开启 IPv6 转发，避免清理出口的 RA 默认路由；兼容 `eth0.100` 等含点号的接口名。回滚混合接口转发状态时临时保护 RA，最后恢复其原值。

### TCP 增强与诊断

- 主菜单 `11` 提供 TFO、ECN + fallback、MTU 黑洞探测的独立启用、关闭和恢复首次基线入口；只修改所选参数，界面显示运行值、保存值及基线。
...
- TFO 需要应用配合，MTU=1 在检测到黑洞后探测；ECN 效果取决于内核、对端和链路，不保证提速。
...
| **同步 V3.12.6** | 新增 TCP 增强独立开关及跨预设偏好；全参数验证、失败与中断回滚；扩展实际队列、配置漂移/来源和网络累计计数诊断；同步主仓增强回归测试 |
| **同步 V3.12.1** | 修复 BBR 主界面将已生效的本工具 tc 限速误报为“已保存，未生效”：确认 HTB root、`class 1:10` 与 `fq 100:` 拓扑后读取实际 `rate/maxrate`，并在进入主界面时尝试恢复确实丢失的已保存规则 |
