...
6. 每次主仓脚本版本更新必须一并构建、校验、发布同版本的 SSH-Hardening 离线包，并同步主仓 README 的 GitHub 直连及 `https://gh-proxy.org/` 加速链接（包含 SHA256）。Release 工作流成功、下载校验通过后才算完成交付；不能仅推送源码。BBR-tune 不另建离线安装器。
