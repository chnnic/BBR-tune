...
manual_buf_apply() {
    local MEM_MB=$1
    local MEM_LABEL=$2
    local RMEM WMEM ADV_WIN NOTSENT TCP_RMEM_DEFAULT BUF_MB

    while true; do
        clear
        echo ""
        echo -e "${BOLD}手动选择缓冲区大小 — 内存：${MEM_LABEL}${NC}"
        echo ""
        echo -e "  ${BOLD}1.${NC} 12 MB  （BDP ≤ 8MB，200Mbps×400ms）"
        echo -e "  ${BOLD}2.${NC} 16 MB  （BDP ≤ 10MB）"
        echo -e "  ${BOLD}3.${NC} 20 MB  （BDP ≤ 13MB，500Mbps×200ms）"
        echo -e "  ${BOLD}4.${NC} 40 MB  （BDP ≤ 26MB，1Gbps×200ms）"
        echo -e "  ${BOLD}5.${NC} 64 MB  （BDP ≤ 42MB，2Gbps×160ms）"
        echo -e "  ${BOLD}6.${NC} 128 MB （BDP ≤ 85MB，2Gbps×340ms）"
        echo -e "  ${BOLD}0.${NC} 返回上级"
        echo ""
        read -rp "请选择 [0-6]：" BUF_CHOICE

        case $BUF_CHOICE in
            1) RMEM=12582912;  WMEM=12582912;  ADV_WIN=2; NOTSENT=131072;  BUF_MB=12  ;;
            2) RMEM=16777216;  WMEM=16777216;  ADV_WIN=2; NOTSENT=131072;  BUF_MB=16  ;;
            3) RMEM=20971520;  WMEM=20971520;  ADV_WIN=2; NOTSENT=131072;  BUF_MB=20  ;;
            4) RMEM=41943040;  WMEM=41943040;  ADV_WIN=3; NOTSENT=262144;  BUF_MB=40  ;;
            5) RMEM=67108864;  WMEM=67108864;  ADV_WIN=3; NOTSENT=524288;  BUF_MB=64  ;;
            6) RMEM=134217728; WMEM=134217728; ADV_WIN=3; NOTSENT=524288;  BUF_MB=128 ;;
            0) return ;;
        ...
        echo ""
        echo -e "${YELLOW}配置摘要：${NC}"
        echo -e "  内存：${MEM_LABEL}  缓冲区：${BUF_MB}MB（手动选择）"
        echo -e "  rmem/wmem max：${BUF_MB}MB"
        echo -e "  adv_win_scale：${ADV_WIN}"
        ...
        if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
            ...
            apply_sysctl "$CONFIG"
            echo ""
            echo -e "${GREEN}✓ BBR TCP 调优配置完成（手动缓冲区 ${BUF_MB}MB）${NC}"
            echo -e "${YELLOW}提示：建议配合限速设置使用，避免 Retr 爆炸${NC}"
        else
            echo -e "${YELLOW}已取消${NC}"
        fi
...
menu_bbr() {
    while true; do
        ...
        case $MEM_CHOICE in
            1) menu_bbr_mode 512  "512MB"; break ;;
            2) menu_bbr_mode 1024 "1GB";   break ;;
            3) menu_bbr_mode 2048 "2GB";   break ;;
            8) backup_sysctl ;;
            9) restore_sysctl ;;
            0) return ;;
            *) echo -e "${RED}无效选项${NC}" ;;
...
menu_bbr_mode() {
    ...
    while true; do
        clear
        echo ""
        echo -e "${BOLD}内存：${MEM_LABEL} — 请选择配置模式${NC}"
        echo ""
        echo -e "  ${BOLD}1.${NC} 自动计算（根据延迟和带宽自动推算缓冲区）"
        echo -e "  ${BOLD}2.${NC} 手动选择（直接指定缓冲区大小）"
        echo -e "  ${BOLD}0.${NC} 返回上级"
        echo ""
        read -rp "请选择 [0-2]：" MODE_CHOICE

        case $MODE_CHOICE in
            1) menu_bbr_latency "$MEM_MB" "$MEM_LABEL"; break ;;
            2) manual_buf_apply "$MEM_MB" "$MEM_LABEL"; break ;;
            0) return ;;
            *) echo -e "${RED}无效选项${NC}" ;;
        esac
    done
