...
bbr_restore_runtime_snapshot() {
    ...
    done < <(bbr_runtime_plan "$(cat "$SNAPSHOT")")
    return "$FAILED"
}

# 转发开关可能重置其他网络参数，因此先恢复/应用转发，再处理其余键。
bbr_runtime_plan() {
    printf '%s\n' "$1" | awk -F= '
        /^[[:space:]]*[#;]/ || !/=/ {next}
        ...
        END {
            for(i=1;i<=n;i++) if(order[i] ~ /^(net\.ipv4\.ip_forward|net\.ipv6\.conf\.all\.forwarding)$/) print order[i] " = " values[order[i]]
            for(i=1;i<=n;i++) if(order[i] !~ /^(net\.ipv4\.ip_forward|net\.ipv6\.conf\.all\.forwarding)$/) print order[i] " = " values[order[i]]
        }
    '
}
...
bbr_tcp_config() {
    local CURRENT="" GROUP KEY VALUE
    [ ! -f "$SYSCTL_FILE" ] || CURRENT=$(cat "$SYSCTL_FILE")
...
bbr_apply_sysctl() (
    ...
    local SKIPPED=0 RUNTIME="" PATHNAME ACTUAL
    ensure_sysctl || return 1
    mkdir -p "$(dirname "$SYSCTL_FILE")" || return 1
    # 同一配置文件只能有一个应用事务。退出或被信号中断时也回滚。
    local TX_LOCK="${SYSCTL_FILE}.lock"
    mkdir "$TX_LOCK" 2>/dev/null || { error "另一个 BBR 参数事务正在运行（${TX_LOCK}）"; return 1; }
    trap '
        if [ "$TX_DIRTY" = 1 ]; then
    ...
    if [ -n "$RUNTIME" ]; then
        bbr_runtime_snapshot "$TX_SNAPSHOT" "$RUNTIME" config || { error "无法保存运行快照"; return 1; }
    fi
    ...
    # 某些 sysctl（如 ip_forward）会重置其他键，再核对最终整体状态。
    while IFS= read -r KEY; do
        VALUE=$(printf '%s\n' "$RUNTIME" | awk -F= -v key="$KEY" '
            { lhs=$1; gsub(/^[[:space:]]+|[[:space:]]+$/, "", lhs) }
            lhs==key {v=$0; sub(/^[^=]*=/, "", v)} END {print v}' | bbr_sysctl_normalize)
        ACTUAL=$(sysctl -n "$KEY" 2>/dev/null) || { error "最终状态不可读：${KEY}"; return 1; }
        [ "$(printf '%s\n' "$ACTUAL" | bbr_sysctl_normalize)" = "$VALUE" ] || {
            error "最终状态不一致：${KEY}（可能被其他参数重置）"
            return 1
        }
    done < <(bbr_config_keys "$RUNTIME" | awk '!seen[$0]++')
