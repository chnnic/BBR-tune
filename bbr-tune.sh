...
bbr_tcp_value_description() {
    # 只解释状态，不把“允许协商/按需探测”当成连接已实际使用该功能。
    # 含额外位标志或未来内核取值时保留原数字，不误判为关闭。
    case "$1:$2" in
        net.ipv4.tcp_fastopen:0) echo '已关闭' ;;
        net.ipv4.tcp_fastopen:1) echo '仅客户端开启' ;;
        net.ipv4.tcp_fastopen:2) echo '仅服务端开启' ;;
        net.ipv4.tcp_fastopen:3) echo '已开启：客户端 + 服务端' ;;
        net.ipv4.tcp_fastopen:*) echo '其他取值；需核对 TFO 位标志' ;;
        net.ipv4.tcp_ecn:0) echo '已关闭' ;;
        net.ipv4.tcp_ecn:1) echo '已开启：入站 + 出站 ECN' ;;
        net.ipv4.tcp_ecn:2) echo '仅入站 ECN；出站不主动启用' ;;
        net.ipv4.tcp_ecn:3) echo '已开启：入站 + 出站 AccECN' ;;
        net.ipv4.tcp_ecn:4) echo '已开启：入站 AccECN / 出站 ECN' ;;
        net.ipv4.tcp_ecn:5) echo '仅入站 AccECN；出站不主动启用' ;;
        net.ipv4.tcp_ecn_fallback:0) echo '已关闭异常回退' ;;
        net.ipv4.tcp_ecn_fallback:1) echo '已开启：ECN 异常时允许回退' ;;
        net.ipv4.tcp_mtu_probing:0) echo '已关闭' ;;
        net.ipv4.tcp_mtu_probing:1) echo '已开启按需模式：检测到黑洞后探测' ;;
        net.ipv4.tcp_mtu_probing:2) echo '始终探测' ;;
        *) echo '未知取值；请核对内核文档' ;;
    esac
}

bbr_tcp_recommendation() {
    case "$1" in
        net.ipv4.tcp_fastopen) echo '3（客户端 + 服务端；需应用支持）' ;;
        net.ipv4.tcp_ecn) echo '1（主动启用增强时；配合 fallback=1）' ;;
        net.ipv4.tcp_ecn_fallback) echo '1（允许异常回退；ECN 关闭时不生效）' ;;
        net.ipv4.tcp_mtu_probing) echo '1（按需探测，非始终探测）' ;;
        *) return 1 ;;
    esac
}
...
bbr_tcp_status() {
    ...
    for GROUP in TFO ECN MTU; do
        for KEY in $(bbr_tcp_keys "$GROUP"); do
            ...
            printf '  %s\n' "$KEY"
            if CURRENT=$(sysctl -n "$KEY" 2>/dev/null) && [ -n "$CURRENT" ]; then
                printf '    当前: %s（%s）\n' "$CURRENT" "$(bbr_tcp_value_description "$KEY" "$CURRENT")"
            else
                printf '    当前: 不支持或不可读（无法判断开关状态）\n'
            fi
            printf '    推荐: %s\n    已保存: %s · 首次基线: %s\n' \
                "$(bbr_tcp_recommendation "$KEY")" "$SAVED" "$BASE"
        done
    done
}
...
bbr_tcp_menu() {
    ...
    while true; do
        ...
        bbr_tcp_status
        echo ""
        echo "  推荐值是本工具的手动增强建议，不会自动修改当前参数。"
        echo "  开启表示内核配置允许，不代表每条连接已使用或一定提速。"
        echo "  TFO 需应用配合；MTU=1 是按需探测，不是关闭。"
        echo "  ECN 需对端和链路兼容；默认保留系统策略，启用时搭配回退。"
        ...
        case "$GROUP" in
            TFO) menu_item "1" "启用（推荐 3：客户端 + 服务端）" ;;
            ECN) menu_item "1" "启用（增强建议：ECN=1 + fallback=1）" ;;
            MTU) menu_item "1" "启用按需探测（推荐 1）" ;;
        esac
        if [ "$GROUP" = ECN ]; then
            menu_item "2" "关闭 ECN（ECN=0；保留 fallback=1）"
        else
            menu_item "2" "关闭（0）"
        fi
...
bbr_standalone_menu() {
    ...
    while true; do
        ...
        menu_item "11" "TCP 增强（TFO / ECN / MTU）"
